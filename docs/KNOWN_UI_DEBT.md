...
- **PERF-1 · Chart.js (~200KB) loads render-blocking in `base.html` `<head>` on every page** — fixed the cheap wins (font `@import`→`<link>` + `preconnect` for fonts & jsdelivr). Chart.js itself is still eager + render-blocking: it's needed by the **compact-view** sparklines (`compact_table.js` `new Chart`) and the **detail** chart, but NOT the default **grid** or **changelog**. Follow-up (needs care): either `defer` all chart scripts in order (base Chart.js + the detail's zoom/adapter/utils/commodity.js) so they stay correctly sequenced, OR switch the compact sparklines to the lightweight custom-canvas `sparkline.js` the grid already uses, which lets Chart.js move to detail-only. Deferred — ordering/interaction risk on the hero chart.
