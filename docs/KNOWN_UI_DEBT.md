...
- **UI-15b · Compare overlays align by index, not date** — with the new time axis, a compared commodity's points still plot at the primary's label dates (datasets share `labels`). For true cross-commodity date alignment, give each dataset `data:[{x:date,y:value}]`. Cosmetic; dual-axis compare is still readable.
