"""
USDA NASS QuickStats API fetcher.
Requires API key from https://quickstats.nass.usda.gov/api
Provides US agricultural commodity prices, production data, and livestock stats.

Reference: https://quickstats.nass.usda.gov/api
"""

import os
import logging
from typing import Any, Dict, List, Optional

from scripts.fetchers._shared import Observation, safe_get

logger = logging.getLogger(__name__)

USDA_API_BASE = "https://quickstats.nass.usda.gov/api/api_GET/"

_MONTH_MAP: Dict[str, str] = {
    "JAN": "01", "FEB": "02", "MAR": "03", "APR": "04",
    "MAY": "05", "JUN": "06", "JUL": "07", "AUG": "08",
    "SEP": "09", "OCT": "10", "NOV": "11", "DEC": "12",
    "YEAR": "06",           # Annual → mid-year
    "MARKETING YEAR": "06",
}

_SKIP_VALUES = frozenset({"(D)", "(NA)", "(S)", "(Z)", ""})


def _get_usda_api_key() -> str:
    """Read USDA_API_KEY at call time so env changes are picked up."""
    return os.getenv('USDA_API_KEY', '')


def _resolve_month(rec: Dict[str, Any]) -> Optional[str]:
    """Resolve a two-digit month string from a NASS record, or None."""
    ref_period = rec.get("reference_period_desc", "").upper()
    month = _MONTH_MAP.get(ref_period)
    if month:
        return month
    # Fallback: try begin_code
    begin_code = rec.get("begin_code", "")
    if begin_code and begin_code.isdigit():
        return begin_code.zfill(2)
    return None


def _parse_usda_records(records: List[Dict[str, Any]]) -> List[Observation]:
    """Convert raw NASS records into [{date, price}] observations, oldest-first."""
    seen: Dict[str, Observation] = {}
    for rec in records:
        val = rec.get("Value", "").replace(",", "").strip()
        if not val or val in _SKIP_VALUES:
            continue
        try:
            price = float(val)
        except (ValueError, TypeError):
            continue

        year = rec.get("year", "")
        month = _resolve_month(rec)
        if not month:
            continue

        dt = f"{year}-{month}-01"
        seen[dt] = {"date": dt, "price": round(price, 4)}

    return sorted(seen.values(), key=lambda x: x["date"])


def fetch_usda_nass(
    commodity_desc: str,
    statisticcat_desc: str = "PRICE RECEIVED",
    unit_desc: str = "$ / BU",
    freq_desc: str = "MONTHLY",
    agg_level_desc: str = "NATIONAL",
    year_start: int = 2000,
    **extra_params: Any,
) -> Optional[List[Dict[str, Any]]]:
    """
    Fetch commodity data from USDA NASS QuickStats API.

    Args:
        commodity_desc: e.g., 'CORN', 'WHEAT', 'SOYBEANS', 'CATTLE'
        statisticcat_desc: e.g., 'PRICE RECEIVED', 'PRODUCTION'
        unit_desc: e.g., '$ / BU', '$ / CWT', '$ / LB'
        freq_desc: 'MONTHLY', 'ANNUAL', 'WEEKLY'
        agg_level_desc: 'NATIONAL', 'STATE'
        year_start: earliest year to fetch
        **extra_params: additional NASS API params

    Returns:
        List of {date, price} dicts, oldest first. None on failure.
    """
    api_key = _get_usda_api_key()
    if not api_key:
        logger.warning("  Missing USDA_API_KEY")
        return None

    params: Dict[str, Any] = {
        "key": api_key,
        "commodity_desc": commodity_desc,
        "statisticcat_desc": statisticcat_desc,
        "unit_desc": unit_desc,
        "freq_desc": freq_desc,
        "agg_level_desc": agg_level_desc,
        "year__GE": str(year_start),
        "format": "JSON",
    }
    params.update(extra_params)

    try:
        resp = safe_get(USDA_API_BASE, params=params)
        records = resp.json().get("data", [])

        if not records:
            logger.warning(f"  No USDA data for {commodity_desc}")
            return None

        results = _parse_usda_records(records)
        return results if results else None

    except Exception as e:
        logger.error(f"  Error fetching USDA {commodity_desc}: {e}")
        return None
